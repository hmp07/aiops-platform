...
async def get_bootstrap(current_user: dict = Depends(get_current_user)):
    """Return agent config, suggested questions, permissions."""
    async with async_session_factory() as db:
        config = (await db.execute(select(AIOpsAgentConfig).limit(1))).scalar_one_or_none()
        return {
            "enabled": True,
            "permissions": current_user.get("permissions", []),
            "suggested_questions": config.suggested_questions if config else [
                "当前有多少设备？", "最近有哪些告警？", "查询 Zabbix server 的状态"
            ],
            "runtime": {
